...
- (void)bulkDelete
{
    ...
    if (openResult == SQLITE_OK)
    {
        for (NSIndexPath *chosenRowIndexPath in bulkSet)
        {
            ...
            for (int i = 0; i < [[picturesArray objectAtIndex:chosenRowIndexPath.row] intValue]; i++)
            {
                ...
                [fileManager removeItemAtPath:[[PICTURES stringByAppendingString:[idArray objectAtIndex:chosenRowIndexPath.row]] stringByAppendingFormat:@"-%d.jpg", i] error:&error];
                if (error) NSLog(@"SMSNinja: Failed to delete %@, error %@", [[PICTURES stringByAppendingString:[idArray objectAtIndex:chosenRowIndexPath.row]] stringByAppendingFormat:@"-%d.png", i], [error localizedDescription]);
            }
        }
    ...
    for (NSIndexPath *chosenRowIndexPath in bulkSet)
    {
        ...
        [readArray removeObjectAtIndex:chosenRowIndexPath.row];
...
- (void)loadDatabaseSegment
{
	...
	if (openResult == SQLITE_OK)
	{
		NSString *sql = [NSString stringWithFormat:@"select name, content, time, number, read, id, pictures from blockedsms order by (cast(id as integer)) desc limit %d, 50", [idArray count] + 1];

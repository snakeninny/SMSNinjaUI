...
- (SNBlockedMessageHistoryViewController *)init
{
	if ((self = [super initWithStyle:UITableViewStylePlain]))
	{
		...
		[backButton addTarget:self action:@selector(gotoMainViewController) forControlEvents:UIControlEventTouchUpInside];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
	...
	switch (buttonIndex)
	{
		case 0:
			...
			[self.tableView beginUpdates];
			[self.tableView deleteRowsAtIndexPaths:[NSArray arrayWithObject:[NSIndexPath indexPathForRow:chosenRow inSection:0]] withRowAnimation:UITableViewRowAnimationFade];
			[self.tableView endUpdates];
			break;
		case 1: // unread
            [bulkSet removeAllObjects];
			...
			for (UIView *view in [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:chosenRow inSection:0]].contentView.subviews)
				if ([view isKindOfClass:[UILabel class]]) ((UILabel *)view).textColor = [UIColor blueColor];
			break;
		case 2: // read
            [bulkSet removeAllObjects];
